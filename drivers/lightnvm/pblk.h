...
#define WR_SEM_MAX  32
#define TOS_PR_RDONLY 1
#define TOS_PR_WRONLY 2
#define TOS_PR_ERONLY 3
#define TOS_PR_ALL    4
...
struct pblk_lun {
    ...
    /* Coperd: add my own per-lun counters here */
    atomic_t inf_wr_freq[WR_SEM_MAX + 1];
    atomic_t inf_wr;
    atomic_t inf_er;
    atomic_t inf_rd;
    atomic_t inf_rd_pgs;
    ...
    struct ppa_addr last_wr_addr;
    ktime_t last_wr_stime; /* Coperd: record the start time of last write */
    u64 last_wr_lat_us;
    ktime_t last_er_stime;
    u64 last_er_lat_us;
    ktime_t last_rd_stime;
    u64 last_rd_lat_us;

    u32 *wr_lat_tbl;
    u64 plat_us;
    struct list_head inf_rqds;
    ktime_t next_avail_time;
    spinlock_t lock;
};
...
extern struct pinfo *rqst;
extern volatile int rq_idx;
extern spinlock_t mt_lock;
...
struct pblk {
    ...
    /* Coperd: my own counters */
    atomic_t nr_tgt_rds;
    atomic_t nr_tgt_user_rds;
    atomic_t nr_tgt_wrs;
    atomic_t nr_tgt_ers;
    atomic_t nr_cc_rds;         /* # of bios entirely hitting the cache */
    atomic_t nr_tt_busy;
#endif
    ...
    /* Coperd: tos related fields */
    int tos_debug;
    int tos_debug_freq; /* Coperd: output debug info every this number reads */
    int tos_switch;
    int tos_tgt_lat;    /* Coperd: target SLO latency in microseconds */
    ...
    int tos_nr_wr_sem_max;
    int tos_pr_lat;
    int tos_def_hflag;
    int tos_gc_switch;
    atomic_t tos_rqd_id;
    atomic_t tos_nr_type1_errs;
    atomic_t tos_nr_type2_errs;
    atomic_t tos_nr_type1_cors;
    atomic_t tos_nr_type2_cors;

    struct chnl_inf *chnl_inf;
    int wr_lat_tbl[512];
};
...
/*
 * Coperd: perlun inflight counter operations
 */

void down_perlun_inf_rd(struct pblk *pblk, struct nvm_rq *rq);
void down_perlun_inf_wr(struct pblk *pblk, struct nvm_rq *rq);
...
void nvm_map_to_dev(struct nvm_tgt_dev *tgt_dev, struct ppa_addr *p);
...
static inline u64 pblk_ppa_to_lun(struct pblk *pblk, struct ppa_addr p)
{
    struct nvm_geo *geo = &pblk->dev->geo;
    return p.g.ch * geo->luns_per_chnl + p.g.lun;
}
